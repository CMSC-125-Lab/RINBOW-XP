...
public class ResourceManager {
    ...
    public ImageIcon getImageIcon(String key) {
        // Return from strong cache if already loaded
        if (imageIconHashMap.containsKey(key)) {
            ImageIcon cached = imageIconHashMap.get(key);
            if (cached == null) {
                throw new RuntimeException("ERROR: Key is null in Image Icon Hashmap: " + key);
            }
            return cached;
        }

        // Lazy-load stageX.gif  (stage0 .. stage8)
        if (key.startsWith("stage") && !key.contains("/")) {
            try {
                String fileName = key + ".gif";
                ImageIcon icon = new ImageIcon(getURLFromFiles("sprites/" + fileName));
                imageIconHashMap.put(key, icon);
                System.out.println("[ResourceManager] Lazy-loaded: sprites/" + fileName);
                return icon;
            } catch (RuntimeException e) {
                System.err.println("[ResourceManager] Failed to load stage GIF: " + key);
                throw new RuntimeException("ERROR: Could not load stage GIF: " + key, e);
            }
        }

        // Lazy-load transitionX.gif  (transition0 .. transition7)
        if (key.startsWith("transition") && !key.contains("/")) {
            try {
                String fileName = key + ".gif";
                ImageIcon icon = new ImageIcon(getURLFromFiles("sprites/" + fileName));
                imageIconHashMap.put(key, icon);
                ...
                System.err.println("[ResourceManager] Failed to load transition GIF: " + key);
                throw new RuntimeException("ERROR: Could not load transition GIF: " + key, e);
            }
        }

        throw new RuntimeException("ERROR: Key not found in Image Icon Hashmap: " + key);
    }
}